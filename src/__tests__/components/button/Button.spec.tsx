...
describe('Button Component', () => {
    it('renders correctly', () => {
        const { getByText } = renderWithTheme(<Button title="CRIAR" handleOnPress={() => {}} />);
    ...
    it('calls handleOnPress when pressed', () => {
        ...
        const { getByText } = renderWithTheme(<Button title="CRIAR" handleOnPress={handleOnPress} />);
        fireEvent.press(getByText('CRIAR'));
        expect(handleOnPress).toHaveBeenCalled();
    });

    it('applies the correct width', () => {
        const { getByTestId } = renderWithTheme(<Button title="CRIAR" width={200} handleOnPress={() => {}} />);
